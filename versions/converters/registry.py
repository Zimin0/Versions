...
from versions.converters.errors import (
    ConverterDoesNotImplementedError,
...
class ConvertersRegistry:
    ...
    def __validate_before_append(self, appended_converter: "Converter"):
        ...
        for converter in self.__all_converters:
            same_source = converter.SOURCE_TYPE is appended_converter.SOURCE_TYPE
            same_target = converter.TARGET_TYPE is appended_converter.TARGET_TYPE
            same_priority = converter.PRIORITY == appended_converter.PRIORITY
    ...
    def find_converters(
        self, in_type: "Version", out_type: "Version"
    ) -> list["Converter"]:
        """
        ...
        * If does not exist - raise a ConverterDoesNotImplementedError error.
        ...
        if len(self.__all_converters) == 0:
            raise ConverterDoesNotImplementedError(
                "Could not find any converters (none were created). "
                "Create a required one: "
                "class `Format1ToFormat2Converter(Converter)`: ..."
            )

        def source_target_filter_func(elem):
            return in_type == elem.SOURCE_TYPE and out_type == elem.TARGET_TYPE

        sorted_converters_by_priority = sorted(
            self.__all_converters, key=lambda c: c.PRIORITY
        )
        filtered_converters = list(
            filter(source_target_filter_func, sorted_converters_by_priority)
        )
        ...
        raise ConverterDoesNotImplementedError(
            f"Converter for transition {in_type!r} to {out_type!r} "
            "does not implemented. "
            "To create a converter: class `Format1ToFormat2Converter(Converter)`: ..."
        )
